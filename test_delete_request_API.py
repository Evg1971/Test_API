...
def test_delete_and_filter_places():
    ...
    assert place_ids and len(place_ids) >= 5, ("Не удалось прочитать place_id из файла "
    ...
    for index in [1, 3]:  # Индексы 1 и 3 соответствуют 2-му и 4-му элементам
        ...
        try:
            ...
            assert response.status_code == 200, f"Не удалось удалить место с place_id: {place_id}"
    ...
    assert existing_places, "Нет существующих мест после удаления."
